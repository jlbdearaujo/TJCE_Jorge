...
Jorge159=pd.read_csv('CLAUDIO159.csv.zip',sep=';', compression='zip')
Jorge436=pd.read_csv('CLAUDIO436.csv.zip',sep=';', compression='zip')
Jorge1116=pd.read_csv('CLAUDIO1116.csv.zip',sep=';', compression='zip')
...
if paginaseleciona=='Execução de Título Extrajudicial':
	...
	if selemv!='NONE':
		...
		ii=laux[laux['MOVIMENTAÇÃO']==selemv].index[0]
		laux.drop(ii,inplace=True)
